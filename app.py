...
camera = None  # Global variable to hold camera instance
...
def update():
    ...
    while True:
        ...
        for face in faces:
            ...
            if lip_counter < 10 and capture_lip_shape:
                # Save the cropped lip image and its corresponding landmarks
                cv2.imwrite(f"{output_folder}/lip_{image_counter}_{lip_counter}.jpg", lip_crop)
                ...
                with open(f"{output_folder}/lip_landmarks_{image_counter}_{lip_counter}.txt", 'w') as file:
